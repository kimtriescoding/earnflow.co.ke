...
const WHEEL_PX = 300;
const BEZEL_PX = 11;
const INNER_PX = WHEEL_PX - BEZEL_PX * 2;
...
export function LuckySpinWheel({ segmentCount, wheelRotation, loading }) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`Lucky spin wheel with 0x through ${segmentCount}x`}
      aria-busy={loading}
    >
      {/* Ambient glow */}
      <div
        ...
        >
          ...
          {rivetAngles.map((deg) => (
            <span
              ...
              style={{
                transform: `translate(-50%, -50%) rotate(${deg}deg) translateY(calc(-${WHEEL_PX / 2 - 4}px))`,
              }}
              aria-hidden
            />
          ))}
          ...
          <div
            ...
            style={{
              width: INNER_PX,
              height: INNER_PX,
              marginLeft: -INNER_PX / 2,
              marginTop: -INNER_PX / 2,
      ...
      <div
        ...
        aria-hidden
      >
        <div className="relative flex flex-col items-center drop-shadow-[0_4px_0_rgba(0,0,0,0.35),0_10px_18px_rgba(0,0,0,0.55)]">
          <div
